...
type RetentionHandler struct {
	RetentionCtl retention.APIController
}
...
func (r *RetentionHandler) constructRetentionPayload(event *event.RetentionEvent) (*model.Payload, bool, int64, error) {
	task, err := r.RetentionCtl.GetRetentionExecTask(event.TaskID)
	if err != nil {
	...
	execution, err := r.RetentionCtl.GetRetentionExec(task.ExecutionID)
	if err != nil {
	...
	md, err := r.RetentionCtl.GetRetention(execution.PolicyID)
	if err != nil {
