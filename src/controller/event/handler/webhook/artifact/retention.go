...
var RetentionController = NewRetentionController()
...
func constructRetentionPayload(event *event.RetentionEvent) (*model.Payload, bool, int64, error) {
	task, err := RetentionController.GetRetentionExecTask(event.TaskID)
	if err != nil {
	...
	execution, err := RetentionController.GetRetentionExec(task.ExecutionID)
	if err != nil {
	...
	md, err := RetentionController.GetRetention(execution.PolicyID)
	if err != nil {
